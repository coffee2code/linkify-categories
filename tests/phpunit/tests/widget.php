...
class Linkify_Widget_Test extends WP_UnitTestCase {
	...
	private function widget_init( $config = array() ) {
		c2c_LinkifyCategoriesWidget::register_widget();
		$widget = new c2c_LinkifyCategoriesWidget();
	...
	public function test_widget_version() {
		$this->assertEquals( '006', c2c_LinkifyCategoriesWidget::version() );
	}
	...
	public function test_widget_made_available() {
		$this->assertArrayHasKey( 'c2c_LinkifyCategoriesWidget', $GLOBALS['wp_widget_factory']->widgets );
	}

	public function test_get_config() {
		list( $widget, $config, $settings ) = $this->widget_init( array( 'categories' => $this->cat_ids[0], 'before' => 'Category: ', 'after' => '.' )  );

		$this->assertEqualsCanonicalizing(
			[ 'after', 'before', 'before_last', 'between', 'categories', 'none', 'title', ],
			array_keys( $widget->get_config() )
		);

		$this->assertEqualsCanonicalizing(
			[ 'default', 'help', 'input', 'label' ],
			array_keys( $widget->get_config()['categories'] )
		);
	}
	...
	public function test_widget_body() {
		...
		$this->assertEquals(
			sprintf( '<div class="my-widget"><h3>Categories</h3>Category: %s.</div>', __c2c_linkify_categories_get_category_link( $this->cat_ids[0] ) ),
			$this->get_results( $widget, $settings, $config )
		);
	}

	public function test_widget_body_with_multiple_categories() {
		list( $widget, $config, $settings ) = $this->widget_init( array( 'categories' => $this->cat_ids[0] . ',' . $this->cat_ids[1], 'before' => '<ul><li>', 'after' => '</li></ul>', 'between' => '</li><li>' )  );

		$this->assertEquals(
			sprintf(
				'<div class="my-widget"><h3>Categories</h3><ul><li>%s</li><li>%s</li></ul></div>',
				__c2c_linkify_categories_get_category_link( $this->cat_ids[0] ),
				__c2c_linkify_categories_get_category_link( $this->cat_ids[1] )
			),
			$this->get_results( $widget, $settings, $config )
		);
	}

	public function test_widget_body_with_invalid_categories() {
	...
	public function test_widget_body_with_no_specified_categories() {
	...
	public function test_output_omits_unsafe_markup() {
		list( $widget, $config, $settings ) = $this->widget_init( array(
			'categories' => array_slice( $this->cat_ids, 0, 2 ),
		...
		$this->assertEquals(
			sprintf(
				'<div class="my-widget">alert("boom!");<h3>alert("boom!");Categoriesalert("boom!");</h3><ul><li>alert("boom!");%s</li><li>alert("boom!");<strong><span class="test">Hi</span></strong></li><li>%salert("boom!");</li></ul>alert("boom!");</div>',
				__c2c_linkify_categories_get_category_link( $this->cat_ids[0] ),
				__c2c_linkify_categories_get_category_link( $this->cat_ids[1] )
			),
			$this->get_results( $widget, $settings, $config )
